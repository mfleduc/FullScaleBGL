...
for nn = 1:ncvs
 ...
 %imaginary values on the diagonal sometimes        
        ...
        covmodel{nn} = @(x)WendlandCov(x, d, 'cov');
...
for l1 = 1:length(lambdas) %% cross-validate for each lambda
    ...
    for nn = 1:ncvs
     ...
     save(sprintf('FSBGL Nugget first/1Wendland/FSBGL results %d levels time %d lambda %.4f covmodel %s.mat',lvls,timendx,lambda,fnname ),...
