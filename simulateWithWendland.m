...
% load(['/glade/work/mleduc/GP emulation/needlet fbgl/FSBGL Nugget first/',...
%     'GC/FSBGL results 3 levels time 1 lambda 0.5000 covmodel gentemp.mat'])
...
lowrank = Phi*csim';
params = np(2,1:end);%[paramrange(1, val(1)),paramrange(2, val(2)), paramrange(3, val(3))];
% clear A;
...
for ii = 1:length(LAT)
    ...
    % d=2*sind(d/2);kern =  gentemp([params(1:2),-0.1],d) ;%+ gentemp([params(3:4),-0.1],d);
...
save('/glade/work/mleduc/GP emulation/needlet fbgl/FSBGL Nugget first/Full scale/sim results.mat','lowrank','highres');
% figure;
% for ii = 1:3
% ax=subplot(2,2,ii); 
...
% caxis([-3,3]);title('Simulation');shading flat;colorbar;%ax.Visible='off';
% end
% ax=subplot(2,2,4);
...
% title('Data');caxis([-3,3]);colorbar;%ax.Visible='off';
%  sgtitle( sprintf('3 Wendlands, Full-scale model') )
